...
Vertex AI connects natively to [BigQuery](https://cloud.google.com/bigquery), a platform that combines SQL-like queries with the ability to pass data to different endpoints for analysis. So once we get our data into BigQuery we can start to run some LLM prompts against it at scale.
...
We can do that with the `CONCAT()` SQL function and the field(s) we want to reference. So assuming we had a table with a field named `animal_type`, here's a query that would tell a joke about whatever creature is stored in that field of each record:
  ...
  ML.GENERATE_TEXT(
    ...
    (
      SELECT
        ...
        animal_type -- Pass value through to results
      FROM
