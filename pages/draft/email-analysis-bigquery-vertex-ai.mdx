---
title: Text analysis at scale with BigQuery and VertexAI
date: 2025-12-12
...
On November 12, 2025, Republicans on the House Oversight Committee [released more than 23,000 documents](https://abcnews.go.com/Politics/house-democrats-release-new-epstein-emails-referencing-trump/story?id=127435983) related to the disgraced financier Jeffrey Epstein.

To help journalists in the newsroom make sense of this many documents at once, the AFP data team used Google's [BigQuery](https://cloud.google.com/bigquery) cloud data platform to process and summarize some of these documents in order to identify newsworthy information more quickly.
...
The key task we want to perform is generating a short summary of the contents of each of the 2,300 emails. If there were fewer emails or we had more time for the task, we could send them one by one to the API of a service like Anthropic or OpenAI for summarization. But since we're interested in performing the task quickly across a large data set, we used Google's [VertexAI platform](https://cloud.google.com/vertex-ai) to take advantage of its scalable compute.

VertexAI connects natively to [BigQuery](https://cloud.google.com/bigquery), a platform that combines SQL-like queries with the ability to pass data to different endpoints for analysis. So once we get our data into BigQuery we can start to run some LLM prompts against it at scale.
...
First we need to connect our BigQuery data to a model available through VertexAI. To do this:
...
Within the Streamlit app, we write a query that joins rows from the email table and summary table on the value of `id`, and return all of the fields that match conditions in the `WHERE` clause.
